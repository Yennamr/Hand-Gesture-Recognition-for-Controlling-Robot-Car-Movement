...
labels = ['Backward', 'Forward', 'Left', 'No-motion', 'Right', 'Stop']
X = []
Y = []
...
for root, dirs, directory in os.walk(path):
    for j in range(len(directory)):
        ...
        if 'Thumbs.db' not in directory[j]:
            ...
            im2arr = np.array(img)
            im2arr = im2arr.reshape(28,28,3) #resize as colur image
...
if os.path.exists('model/model.json'):
    ...
    hist = cnn_model.fit(X_train, y_train, batch_size=32, epochs=15, shuffle=True, verbose=2, validation_data=(X_test, y_test)) #start traing model
